...
def sparsesample_reweight(adj: SparseTensor, deg: int) -> SparseTensor:
    '''
    Sampling elements from an adjacency matrix with reweighting.
    Nodes with degree <= deg keep all neighbours; nodes with degree > deg
    are randomly sampled to deg neighbours, and edge weights are scaled
    by (original_degree / deg) to preserve expected sums.
    '''
...
def spmoverlap_(adj1: SparseTensor, adj2: SparseTensor) -> SparseTensor:
    '''
    Compute the overlap of neighbors (rows in adj). The returned matrix is
    similar to the hadamard product of adj1 and adj2.
    '''
...
def spmoverlap_notoverlap_(
    ...
    '''
    Return (overlap, adj1-only, adj2-only) in a single pass.
    '''
    element1 = spm2elem(adj1)
    element2 = spm2elem(adj2)
